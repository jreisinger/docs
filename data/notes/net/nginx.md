...
* $connection_upgrade - new variable whose value depends on $http_upgrade. If $http_upgrade is empty it gets set to `close`. In all other cases it will be set to `upgrade`. 
